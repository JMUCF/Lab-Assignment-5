...
//INPUT VARIABLES IN CHARACTER CLASS
[System.Serializable]
...
public class SolutionTwo : MonoBehaviour
{
    ...
    void Start()
    {
        hitDie = FindHitdie(character.className);

        if (character.isAveraged)
            totalHealth += RollHpAveraged(hitDie, character.characterLevel);
        else
            totalHealth += RollHp(hitDie, character.characterLevel);
		
		//checking hillDwarf and toughFeat

        if (character.isHillDwarf)
            totalHealth += character.characterLevel;
        if (character.hasToughFeat)
            totalHealth += character.characterLevel * 2;

        totalHealth += ConstitutionCheck(character.conScore); //checking conScore chart

        if (totalHealth <= 0)	//incase total health ends up negative set to 1
            totalHealth = 1;

        Debug.LogFormat("My character {0} is a level {1} {2} with a CON score of {3} and {4} a Hill Dwarf and {5} Tough feat. I want the HP {6}", character.characterName, character.characterLevel, character.className, character.conScore, character.isHillDwarf ? "is" : "is not", character.hasToughFeat ? "has" : "doesn't have", character.isAveraged ? "averaged" : "rolled");
